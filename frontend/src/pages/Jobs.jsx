...
const RefreshIcon = () => (
  ...
  >
    ...
    <path d="M3.51 9a9 9 0 0 1 14.85-3.51L20.49 9M3.51 15l2.98 6.49a9 9 0 0 0 14.85-3.51"></path>
  </svg>
);
...
const JobsComponent = () => {
  ...
  const fetchJobs = async () => {
    ...
    try {
      ...
      setError(
        "No Jobs Found. Try entering Job Role and Location to find jobs."
      );
    } finally {
      ...
      <div className="bg-white/50 backdrop-blur-lg p-6 rounded-xl shadow-md border border-gray-200 mb-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
          ...
          <button
            ...
            className="w-full inline-flex justify-center items-center gap-2 px-4 py-2 rounded-lg transition text-white font-semibold bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 disabled:cursor-not-allowed"
          >
            {loadingScrape ? (
              <>
                <Spinner /> Finding Jobs...
              </>
            ) : (
              <>
          ...
          <button
            ...
            className="w-full inline-flex justify-center items-center gap-2 px-4 py-2 rounded-lg transition text-white font-semibold bg-gray-600 hover:bg-gray-700 disabled:bg-gray-400 disabled:cursor-not-allowed"
          >
            {loadingJobs ? (
              <>
        ...
        {loadingScrape && (
          <div className="mt-4 p-3 bg-blue-100 rounded-lg">
            <h3 className="font-semibold text-blue-800 mb-2 text-center">
              Finding Job in progress...
            </h3>
            {Array.isArray(scrapeTasks) &&
              scrapeTasks.map((task) => (
                <div
                  key={task.jobid}
                  className="text-sm text-blue-700 flex justify-between items-center"
                >
                  <span>
                    {task.spider
                      .replace(/_/g, " ")
      ...
      {sortedJobs.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {sortedJobs.map((job, index) => (
            <Card key={job.id || `${job.source}-${index}`} {...job} />
          ))}
        </div>
      )}
      ...
      {sortedJobs.length === 0 && !loadingJobs && !error && (
        <div className="text-center py-16">
          ...
          <p className="text-gray-500 mt-2">
            Try scraping for new jobs or loading the demo data.
          </p>
        </div>
      )}
    </div>
    ...
    <div className="bg-gray-50 min-h-screen font-sans p-4 sm:p-6 md:p-8">
      <div className="text-center mb-12">
        ...
        <p className="text-lg text-gray-500">
          Use our platform to find the latest job openings.
        </p>
      </div>
      <JobsComponent />
    </div>
