...
const JobsComponent = () => {
  ...
  useEffect(() => {
    if (scrapeTasks.length === 0 || !scrapeTasks.some(t => t.status === 'pending')) return;
      ...
      try {
        const updatedTasks = await Promise.all(
          scrapeTasks.map(async (task) => {
            ...
            const { data } = await axiosInstance.get(`/tasks/scrape/status/${task.jobid}`);
  ...
  return (
    <div className="max-w-7xl mx-auto">
      <div className="bg-white/50 backdrop-blur-lg p-6 rounded-xl shadow-md border border-gray-200 mb-8 sticky top-4 z-10">
