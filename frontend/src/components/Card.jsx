...
const Card = ({ source, url, title, company, location, salary, published }) => {
  ...
  const currentSource = sourceInfo[source.toLowerCase()] || {
  ...
  return (
    <>
      {title &&(
        <div
          ref={cardRef}
          className="group relative bg-white border border-gray-200 rounded-xl shadow-sm transition-all duration-300 ease-out will-change-transform"
        ...
        >
          ...
          <div className="relative p-5 flex flex-col h-full">
            <div className="flex items-start justify-between mb-4">
              ...
              <span
                className={`inline-block px-3 py-1 text-xs font-semibold rounded-full ${currentSource.bg} ${currentSource.textColor}`}
              >
                {currentSource.text}
              </span>
            </div>
            ...
            <div className="flex justify-between items-center pt-4 border-t border-gray-100">
              ...
              <a
                ...
                className="inline-flex items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent bg-blue-600 text-white px-4 py-2 opacity-100 translate-y-0 transition-all duration-300 ease-out focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
              >
                Apply Now
              </a>
            </div>
